...
public class UserResource {
    ...
    public ResponseEntity<UserDTO> getCurrentUser() {
        ...
        if(cardNo != null) {
            User user = userService.getUserWithAuthoritiesByLogin(SecurityUtils.getCurrentUserLogin().orElse(null)).orElse(null);
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the "login" user, or with status 404 (Not Found)
     */
    @GetMapping("/current-users-agree")
    @Timed
    public ResponseEntity<UserDTO> saveCurrentUserAgree() {
        log.debug("REST request to get User : {}");
        User user = userService.getUserWithAuthoritiesByLogin(SecurityUtils.getCurrentUserLogin().orElse(null)).orElse(null);
        user.setNdaActivated(true);
        User result = userRepository.save(user);
        return ResponseUtil.wrapOrNotFound(Optional.of(result).map(UserDTO::new));
    }
    
    @GetMapping("/login-description/{id}")
    @Timed
    public String getLoginFullName(@PathVariable String ids) {
        log.debug("REST request to get Login Name : {}", ids);
        String name = userService.GetLoginFullName(ids);

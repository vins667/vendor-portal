...
@Component({
  selector: 'jhi-cut-sch-bundle-productionorder-selection-entry',
  templateUrl: './cut-sch-bundle-productionorder-selection.component.html'
})
export class CutSchBundleProductionorderSelectionComponent implements OnInit {
  isSaving = false;
  isProcess = false;
  ...
  constructor(
    ...
    public cuttingEntryService: CutPlanBundleService
  ) {
  ...
  loadData(productionorder?: IProductionorder): void {
    this.eventManager.broadcast({ name: 'cuttingSchBundleEntryFilter', content: productionorder });
    this.activeModal.dismiss();
  }
}

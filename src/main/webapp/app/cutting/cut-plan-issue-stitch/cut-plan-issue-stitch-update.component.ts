...
@Component({
  selector: 'jhi-cut-plan-issue-stitch-update',
  templateUrl: './cut-plan-issue-stitch-update.component.html'
})
export class CutPlanIssueStitchUpdateComponent implements OnInit {
  isSaving = false;
  ...
  defaultDetails(): void {
    ...
    for (let i = 0; i < 5; i++) {
      this.cutIssueStitchDetails.push(new CutIssueStitchDetails());
    }
  }
      ...
      this.cutPlanIssueStitchService.bundle(Number(this.cutIssueStitchDetails[index].bundleCode)).subscribe(
        cutIssueStitchDetailTemp => {
          if (cutIssueStitchDetailTemp.body && cutIssueStitchDetailTemp.body.id) {
            ...
            } else if (existIndex === this.cutIssueStitchDetails.length) {
              this.cutIssueStitchDetails[index].bundleCode = undefined;
  ...
  post(toast: any): void {
    this.isSaving = true;
    this.snotifyService.remove(toast.id);
    this.subscribeToSaveResponse(this.cutPlanIssueStitchService.post(this.editForm.controls['id'].value), toast);
  }
}

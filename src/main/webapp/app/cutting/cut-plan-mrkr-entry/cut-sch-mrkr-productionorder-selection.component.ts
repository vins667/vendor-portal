...
@Component({
  selector: 'jhi-cut-sch-mrkr-productionorder-selection-entry',
  templateUrl: './cut-sch-mrkr-productionorder-selection.component.html'
})
export class CutSchMrkrProductionorderSelectionComponent implements OnInit {
  isSaving = false;
  isProcess = false;
  ...
  constructor(
    ...
    public cuttingEntryService: CutPlanMrkrEntryService
  ) {
  ...
  loadData(productionorder?: IProductionorder): void {
    this.eventManager.broadcast({ name: 'cuttingScrEntryFilter', content: productionorder });
    this.activeModal.dismiss();
  }
}

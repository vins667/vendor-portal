...
@Component({
  selector: 'jhi-direct-booking-entry',
  templateUrl: './direct-booking-entry.component.html'
})
export class DirectBookingEntryComponent implements OnInit, OnDestroy {
  companies?: ICompany[];
  divisions?: IDivision[];
  factories?: IFactory[];
  ...
  delete(directBookingEntry: IDirectBookingEntry): void {
    const modalRef = this.modalService.open(DirectBookingEntryDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.directBookingEntry = directBookingEntry;
  }
  ...
  protected onSuccess(data: IDirectBookingEntry[] | null, headers: HttpHeaders, page: number): void {
    ...
    this.page = page;
    this.router.navigate(['/direct-booking-entry'], {
      queryParams: {
        page: this.page,

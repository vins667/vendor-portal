...
export class ConveyanceMasterApprovalService {
  ...
  update(conveyanceMaster: IConveyanceMaster): Observable<EntityResponseType> {
    return this.http.put<IConveyanceMaster>(this.resourceUrl + '-approval', conveyanceMaster, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<IConveyanceMaster>(`${this.resourceUrl}-aprv/${id}`, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }

  query(req?: ConveyanceSearchMaster): Observable<EntityArrayResponseType> {
    return this.http
      .post<IConveyanceMaster[]>(this.resourceUrl + '-hod', req, { observe: 'response' })

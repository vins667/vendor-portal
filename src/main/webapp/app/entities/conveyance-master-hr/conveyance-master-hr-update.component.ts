...
@Component({
  selector: 'jhi-conveyance-master-hr-update',
  templateUrl: './conveyance-master-hr-update.component.html',
  providers: [
...
export class ConveyanceMasterHrUpdateComponent implements OnInit {
  isSaving: boolean;
  ...
  download(conveyanceMasterDetails: IConveyanceMasterDetails): any {
    this.conveyanceMasterHrService.download(conveyanceMasterDetails.id).subscribe(
      res => {
  ...
  saveApproved() {
    this.snotifyService.confirm('Are you sure to Approve?', 'Confirm', {
      ...
      buttons: [
        { text: 'Yes', action: toast => this.save('C', toast), bold: false },
  ...
  save(flag: string, toast: any) {
    ...
    if (this.conveyanceMasters.id !== undefined) {
      this.conveyanceMasterHrService.update(this.conveyanceMasters).subscribe((result: HttpResponse<IConveyanceMaster>) => {

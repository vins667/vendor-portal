...
@Component({
  selector: 'jhi-delivery-challan-factory-popup',
  templateUrl: './delivery-challan-factory-popup.component.html'
})
export class DeliveryChallanFactoryPopupComponent implements OnInit, OnDestroy {
  ...
  search() {
    ...
    this.deliveryChallanService
      .queryFactory(this.deliveryChallanSearch)
      .subscribe(
  ...
  addData(factCode: string) {
    this.deliveryChallanService.findByFactory(factCode).subscribe(res => {
      this.deliveryChallan = res.body;
      this.eventManager.broadcast({ name: 'deliveryChallan', content: this.deliveryChallan });
      this.activeModal.dismiss('cancel');
    });
  }
}

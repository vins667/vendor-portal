...
@Component({
  selector: 'jhi-event-others',
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: 'event-others.component.html',
...
export class EventOthersComponent implements OnInit {
  account: Account;
  ...
  eventToDate: any;
  createdDate: string;
  eventAccess: IEventAccess[] = [];
  currentEventAccess: IEventAccess;
  ...
  ngOnInit() {
    this.eventService.queryAccess().subscribe(eventAccess => {
      this.eventAccess = eventAccess.body;
      if (this.eventAccess && this.eventAccess.length > 0) {
        this.currentEventAccess = this.eventAccess[0];
        this.currentCardNo = this.currentEventAccess.id.accessCardNo;
        this.fetchEvents();
      }
    });
  ...
  changeCardNo() {
    this.eventAccess.forEach(eventAccess => {
      if (eventAccess.id.accessCardNo === this.currentCardNo) {
        this.currentEventAccess = eventAccess;
        this.fetchEvents();
      }
    });
  }
  ...
  open(content, subject, startDate?: Date, endDate?: Date) {
    ...
    this.ngbModalRef = this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title', backdrop: 'static' });
  ...
  save() {
    this.isSaving = true;
    this.event.createdBy = this.currentCardNo;
  ...
  fetchEvents() {
    ...
    let prevDate = 0;
    this.eventService.eventMonthAndCardNo(this.viewDate, this.currentCardNo).subscribe(
  ...
  public deleteEvent() {
    if (this.event.id !== undefined) {
      this.eventService.delete(this.event.id).subscribe(response => {
        this.ngbModalRef.dismiss('close');

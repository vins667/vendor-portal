...
@Component({
  selector: 'jhi-vehicle-trp-update',
  templateUrl: './vehicle-trp-update.component.html',
  providers: [
...
export class VehicleTrpUpdateComponent implements OnInit {
  ...
  constructor(
    protected vehicleMasterService: VehicleTrpApprovalService,
  ...
  ngOnInit() {
    ...
    this.activatedRoute.data.subscribe(({ vehicleMaster }) => {
      ...
      if (this.vehicleMaster.flag === 'A') {
        this.vehicleMaster.flag = 'C';
  ...
  save() {
    ...
    if (this.vehicleMaster.id !== undefined) {
      this.subscribeToSaveResponse(this.vehicleMasterService.updateTransport(this.vehicleMaster));
    }
  }

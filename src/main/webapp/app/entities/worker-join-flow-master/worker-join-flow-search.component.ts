...
@Component({
  selector: 'jhi-worker-join-flow-search',
  templateUrl: './worker-join-flow-search.component.html'
})
export class WorkerJoinFlowSearchComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  addData(employeeView) {
    this.eventManager.broadcast({ name: 'selectedUserLinkJoinCreation', content: employeeView });
    this.activeModal.dismiss('cancel');
  }
}

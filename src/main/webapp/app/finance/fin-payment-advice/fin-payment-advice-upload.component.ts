...
@Component({
  selector: 'jhi-fin-payment-advice-upload',
  templateUrl: './fin-payment-advice-upload.component.html',
  styleUrls: ['./fin-payment-advice-upload.component.scss']
})
export class FinPaymentAdviceUploadComponent implements OnInit {
  ...
  save() {
    if (this.selectedFiles!.length > 0) {
      ...
      this.finPaymentAdviceService.upload(this.currentFileUpload).subscribe(
        () => {
  ...
  download(): any {
    this.finPaymentAdviceService.download().subscribe(
      res => {

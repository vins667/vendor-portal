...
@Component({
  selector: 'jhi-job-work-followup-search',
  templateUrl: './job-work-followup-search.component.html'
})
export class JobWorkFollowupSearchComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  registerChangeInEmployeeViews() {
    this.eventSubscriber = this.eventManager.subscribe('selectedUserLinkCreation', response => this.loadAll());
  }

  sort() {
    const result = [this.predicate + ',' + (this.reverse ? 'asc' : 'desc')];
    if (this.predicate !== 'buyercode') {
      result.push('buyercode');
    }
  ...
  addData(employeeView) {
    this.eventManager.broadcast({ name: 'followUpSelectedUserLinkCreation', content: employeeView });
    this.activeModal.dismiss('cancel');
  }
}

...
@Component({
  selector: 'jhi-packing-productionorder-selection-entry',
  templateUrl: './packing-productionorder-selection.component.html'
})
export class PackingProductionorderSelectionComponent implements OnInit {
  isSaving = false;
  isProcess = false;
  ...
  constructor(
    ...
    public cuttingEntryService: PackingProductionEntryService
  ) {
  ...
  loadData(productionorder?: IProductionorder): void {
    this.eventManager.broadcast({ name: 'packingProductionEntryFilter', content: productionorder });
    this.activeModal.dismiss();
  }
}

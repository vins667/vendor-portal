...
import { PaymentRequestApprovalService } from './payment-request-approval.service';
...
@Component({
  selector: 'jhi-payment-request-approval',
  templateUrl: './payment-request-approval.component.html'
})
export class PaymentRequestApprovalComponent implements OnInit, OnDestroy {
  companies?: ICompany[];
  divisions?: IDivision[];
  factories?: IFactory[];
  ...
  constructor(
    protected paymentRequestFormService: PaymentRequestApprovalService,
  ...
  protected onSuccess(data: IPaymentRequestForm[] | null, headers: HttpHeaders, page: number): void {
    ...
    this.page = page;
    this.router.navigate(['/payment-request-approval'], {
      queryParams: {
        page: this.page,

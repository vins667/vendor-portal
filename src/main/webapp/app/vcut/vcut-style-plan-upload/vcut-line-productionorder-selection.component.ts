...
@Component({
  selector: 'jhi-vcut-line-productionorder-selection',
  templateUrl: './vcut-line-productionorder-selection.component.html'
})
export class VcutLineProductionorderSelectionComponent implements OnInit {
  isSaving = false;
  isProcess = false;
  ...
  constructor(
    ...
    public stitchLineIssueService: VcutStylePlanUploadService
  ) {
  ...
  search(): void {
    ...
    this.stitchLineIssueService
      .queryCustomPO(this.productionorderSearch)
  ...
  loadData(productionorder?: IProductionorder): void {
    this.eventManager.broadcast({ name: 'vcutPlanEntryFilter', content: productionorder });
    this.activeModal.dismiss();
  }
}
